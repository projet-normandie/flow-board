...
use App\Presentation\Controller\Admin\BoardController;
...
final class BoardControllerTest extends TestCase
{
    ...
    public function testIndexRendersBoardList(): void
    {
        $boards = [new Board(), new Board()];

        $repository = $this->createStub(BoardRepository::class);
        $repository->method('findAll')->willReturn($boards);

        $this->twig->method('render')->willReturn('<html>boards</html>');

        $response = $this->controller->index($repository);

        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        self::assertSame('<html>boards</html>', $response->getContent());
    }

    public function testIndexPassesBoardsToTemplate(): void
    {
        $boards = [new Board(), new Board()];
        ...
        $twig->expects(self::once())
            ->method('render')
            ->with(
                '@App/admin/boards/index.html.twig',
                ['boards' => $boards],
            )
            ->willReturn('<html>boards</html>');
    ...
    public function testNewPersistsBoardOnValidSubmit(): void
    {
        $form = $this->createStub(FormInterface::class);
        ...
        $this->formFactory->method('create')->willReturn($form);
        $this->urlGenerator->method('generate')->willReturn('/admin/boards');
    ...
    public function testEditRendersFormOnGet(): void
    {
        ...
        $entityManager = $this->createStub(EntityManagerInterface::class);
        $response = $this->controller->edit(new Request(), $board, $entityManager);
    ...
    public function testDeleteRemovesBoardWithValidCsrfToken(): void
    {
        $board = (new Board())->setName('Sprint 1');
        ...
        $response = $this->controller->delete($request, $board, $entityManager);
